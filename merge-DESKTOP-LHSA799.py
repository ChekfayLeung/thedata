        ...
        if True:  # 保存步
            ...
            if any(data.eval("hc==2")):
                data.loc[data.eval("hc==2"), ["CHECK", 'id', 'age', 'sex', 'disease', 'icd10', 'inday', 'dept1',
                                                  ...
                                                  'county', 'hp_name', 'hc',
                                                  'pu-private']]. \
                        to_json(re.sub("\.xls\w?", "_门诊.json", filepath))
            if any(data.eval("hc==1")):
                data.loc[data.eval("hc==1"),
                             ["CHECK", 'id', 'age', 'sex', 'inday', 'outday', 'stayday', 'dept1', 'dept2', 'disease',
                              ...
                              'pu-private']]. \
                        to_json(re.sub("\.xls\w?", "_住院.json", filepath))
            if any(data.eval("hc!=1 and hc!=2")):
                    data[data.eval("hc!=1 and hc!=2")].to_json(re.sub("\.xls\w?", "_没hc.json", filepath))
    ...
    if True:
        data = pd.DataFrame(columns=
                            ['id', 'age', 'sex', "birthday", 'inday', 'outday', 'stayday', 'dept1', 'dept2',
                             ...
                             'pu-private',
                             "CHECK"])
        files = getfile(path, ".json", ["_门诊", "_住院"])
        for file in files:
            data = pd.concat([data, pd.read_json(file)], sort=False)
        ...
        if True:
            ...
            if True:  # 疾病匹配步骤
                ...
                data1 = data.copy()
                data = data['diag'].drop_duplicates().reset_index(drop=True)
                result=[[]]
                ...
                if len(data) > 0:  # 模拟匹配
                    ...
                    Threads = []
                result = pd.merge(data1,pd.concat(result, sort=True),how="left")
